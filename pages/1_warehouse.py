...
with col1:
    ...
    api_key = st.text_input(
        "高德API密钥",
        type="password",
        placeholder="输入您的API密钥进行地理编码",
        help="请到高德开放平台申请API密钥"
    )
    ...
    if geocode_btn and warehouse_address:
        if not api_key:
            ...
            with st.spinner("正在获取坐标..."):
                result = geocode(warehouse_address, api_key)
                if result:
                    lng, lat = result
                    st.session_state.warehouse["lng"] = lng
                    st.session_state.warehouse["lat"] = lat
                    st.session_state.warehouse["address"] = warehouse_address
                    st.session_state.warehouse["name"] = warehouse_name
                    st.session_state.warehouse["capacity_kg"] = capacity_kg
                    st.session_state.warehouse["capacity_m3"] = capacity_m3
...
with col2:
    ...
    if wh.get("lng") and wh.get("lat"):
        st.success("✅ 坐标已设置")
        st.write(f"**名称:** {wh.get('name') or '未命名'}")
        st.write(f"**地址:** {wh.get('address') or '未填写'}")
        st.write(f"**坐标:** ({wh.get('lng'):.6f}, {wh.get('lat'):.6f})")
        st.write(f"**容量:** {wh.get('capacity_kg', 0):,} kg / {wh.get('capacity_m3', 0):,} m³")
    else:
        st.warning("⚠️ 坐标未设置")
...
if st.session_state.warehouse.get("lng") and st.session_state.warehouse.get("lat"):
    ...
    folium.Marker(
        ...
        popup=f"""
        <b>{st.session_state.warehouse.get('name', '仓库')}</b><br>
        地址: {st.session_state.warehouse.get('address', '')}<br>
        容量: {st.session_state.warehouse.get('capacity_kg', 0):,} kg
        """,
...
with col1:
    if st.button("🗑️ 清除仓库数据", use_container_width=True):
        st.session_state.warehouse = {
            "name": "",
            "address": "",
            "lng": None,
