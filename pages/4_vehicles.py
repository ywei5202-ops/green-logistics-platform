        ...
        if new_qty > 0:
            ...
            with col_load:
                actual_load = st.slider(
                    f"实际载重 (吨)",
                    min_value=float(load_min),
                    max_value=float(load_max),
            ...
            with col_ef:
                actual_ef = st.slider(
                    f"碳排放因子 (kg CO₂/吨·km)",
                    min_value=float(ef_min),
                    max_value=float(ef_max),
                    value=float(current_ef),
                    step=0.001,
                    key=f"ef_{vid}",
                    format="%.3f"
                )
            ...
            vehicle_configs[vid] = {
                ...
                "fuel_type": fuel_type,
                "count": new_qty,
                "load_ton": actual_load,
                "emission_factor": actual_ef
            }
        else:
            # 数量为0时不保存但保留滑块位置
            pass
...
# 同步到 session_state
def sync_vehicles():
    """同步配置到 session_state.vehicles"""
    vehicles_list = list(vehicle_configs.values())
    st.session_state.vehicles = vehicles_list
...
total_load_ton = sum(cfg["count"] * cfg["load_ton"] for cfg in vehicle_configs.values())

col_s1, col_s2 = st.columns(2)
with col_s1:
    st.metric("已选车辆种类", len(vehicle_configs))
with col_s2:
...
if vehicle_configs:
    ...
    for cfg in vehicle_configs.values():
        detail_data.append({
            ...
            "数量": cfg["count"],
            "载重/吨": cfg["load_ton"],
            "碳因子": cfg["emission_factor"],
            "小计载重": cfg["count"] * cfg["load_ton"]
        })
...
if st.button("💾 保存配置", type="primary", use_container_width=True):
    sync_vehicles()
    st.success(f"已保存 {len(st.session_state.vehicles)} 种车型配置，共 {total_vehicles} 辆车")

st.caption("💡 提示: 车型库位于 data/vehicle_types.json，可根据实际需求修改")
